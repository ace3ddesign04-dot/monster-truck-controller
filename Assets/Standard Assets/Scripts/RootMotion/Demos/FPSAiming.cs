...
namespace RootMotion.Demos
{
	public class FPSAiming : MonoBehaviour
	{
		...
		private void LookDownTheSight()
		{
			...
			Quaternion rhs = Quaternion.Inverse(gun.rotation) * ik.solver.leftHandEffector.bone.rotation;
			Quaternion rhs2 = Quaternion.Inverse(gun.rotation) * ik.solver.rightHandEffector.bone.rotation;
			float d = 1f;
			ik.solver.leftHandEffector.positionOffset += (gunTarget.TransformPoint(position) - (ik.solver.leftHandEffector.bone.position + ik.solver.leftHandEffector.positionOffset)) * d;
			ik.solver.rightHandEffector.positionOffset += (gunTarget.TransformPoint(position2) - (ik.solver.rightHandEffector.bone.position + ik.solver.rightHandEffector.positionOffset)) * d;
			ik.solver.headMapping.maintainRotationWeight = 1f;
			if (recoil != null)
			{
				recoil.SetHandRotations(gunTarget.rotation * rhs, gunTarget.rotation * rhs2);
			}
			ik.solver.Update();
			if (recoil != null)
			{
				ik.references.leftHand.rotation = recoil.rotationOffset * (gunTarget.rotation * rhs);
				ik.references.rightHand.rotation = recoil.rotationOffset * (gunTarget.rotation * rhs2);
			}
			else
			{
				ik.references.leftHand.rotation = gunTarget.rotation * rhs;
				ik.references.rightHand.rotation = gunTarget.rotation * rhs2;
			}
		...
		private void RotateCharacter()
		{
			...
			if (maxAngle <= 0f)
			{
				...
				float x = forward.x;
				Vector3 forward2 = cam.transform.forward;
				transform.rotation = Quaternion.LookRotation(new Vector3(x, 0f, forward2.z));
				return;
			}
