...
namespace RootMotion.Demos
{
	public class MechSpiderLeg : MonoBehaviour
	{
		...
		{
			stepFound = false;
			Vector3 a = mechSpider.transform.TransformPoint(defaultPosition);
			a += (hit.point - position) * velocityPrediction;
			...
			if (Physics.Raycast(a + up * mechSpider.raycastHeight * mechSpider.scale, -up, out hit, mechSpider.raycastHeight * mechSpider.scale + distance, mechSpider.raycastLayers))
			{
				stepFound = true;
			}
