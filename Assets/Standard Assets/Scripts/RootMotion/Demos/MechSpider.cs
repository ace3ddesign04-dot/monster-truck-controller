...
namespace RootMotion.Demos
{
	public class MechSpider : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			base.transform.rotation = Quaternion.Slerp(base.transform.rotation, lhs * base.transform.rotation, Time.deltaTime * rootRotationSpeed);
			Vector3 legCentroid = GetLegCentroid();
			Vector3 a = Vector3.Project(legCentroid + base.transform.up * height * scale - base.transform.position, base.transform.up);
			base.transform.position += a * Time.deltaTime * (rootPositionSpeed * scale);
			...
			float d = Mathf.Sin(sine) * breatheMagnitude * scale;
			Vector3 b2 = base.transform.up * d;
			body.transform.position = base.transform.position + b2;
		}

		private Vector3 GetLegCentroid()
		{
			Vector3 vector = Vector3.zero;
			float d = 1f / (float)legs.Length;
			for (int i = 0; i < legs.Length; i++)
			{
				vector += legs[i].position * d;
			}
			return vector;
		}
