	...
	public class KissingRig : MonoBehaviour
	{
		...
		public class Partner
		{
			...
			public void Update(float weight)
			{
				...
				float x = ik.solver.bodyEffector.position.x;
				Vector3 position = ik.solver.bodyEffector.bone.position;
				bodyEffector.position = Vector3.Lerp(new Vector3(x, position.y, ik.solver.bodyEffector.position.z), ik.solver.bodyEffector.position, bodyWeightVertical * weight);
				IKEffector bodyEffector2 = ik.solver.bodyEffector;
				Vector3 position2 = ik.solver.bodyEffector.bone.position;
				float x2 = position2.x;
				float y = ik.solver.bodyEffector.position.y;
				Vector3 position3 = ik.solver.bodyEffector.bone.position;
				bodyEffector2.position = Vector3.Lerp(new Vector3(x2, y, position3.z), ik.solver.bodyEffector.position, bodyWeightHorizontal * weight);
			...
			{
				Vector3 b = ik.solver.GetEffector(effector).bone.position - target.position;
				ik.solver.GetEffector(effector).position = Vector3.Lerp(ik.solver.GetEffector(effector).bone.position, targetPosition + b, weight);
			}
		}
