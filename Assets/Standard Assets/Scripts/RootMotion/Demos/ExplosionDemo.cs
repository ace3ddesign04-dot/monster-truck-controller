...
namespace RootMotion.Demos
{
	public class ExplosionDemo : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			if (UnityEngine.Input.GetKeyDown(KeyCode.E) && character.isGrounded)
			{
				...
				vector.y = 0f;
				float d = explosionForceByDistance.Evaluate(vector.magnitude);
				r.velocity = (vector.normalized + Vector3.up * upForce) * d * forceMlp;
			}
