	...
	public class IKSolverLimb : IKSolverTrigonometric
	{
		...
		private Vector3 GetModifiedBendNormal()
		{
			...
			case BendModifier.Parent:
			{
				...
				Quaternion lhs = bone1.transform.parent.rotation * Quaternion.Inverse(parentDefaultRotation);
				return Quaternion.Slerp(Quaternion.identity, lhs * Quaternion.Inverse(defaultRootRotation), num) * bendNormal;
			}
			...
			case BendModifier.Arm:
			{
				...
				Vector3 vector2 = (IKPosition - bone1.transform.position).normalized;
				vector2 = Quaternion.Inverse(bone1.transform.parent.rotation * Quaternion.Inverse(parentDefaultRotation)) * vector2;
				if (goal == AvatarIKGoal.LeftHand)
				{
				...
				{
					axisDirections[i].dot = Mathf.Clamp(Vector3.Dot(axisDirections[i].direction, vector2), 0f, 1f);
					axisDirections[i].dot = Interp.Float(axisDirections[i].dot, InterpolationMode.InOutQuintic);
				}
				Vector3 vector3 = axisDirections[0].axis;
				for (int j = 1; j < axisDirections.Length; j++)
				{
					vector3 = Vector3.Slerp(vector3, axisDirections[j].axis, axisDirections[j].dot);
				}
				if (goal == AvatarIKGoal.LeftHand)
				{
				...
				Vector3 vector4 = bone1.transform.parent.rotation * Quaternion.Inverse(parentDefaultRotation) * vector3;
				if (num >= 1f)
				{
					return vector4;
				}
				return Vector3.Lerp(bendNormal, vector4, num);
			}
