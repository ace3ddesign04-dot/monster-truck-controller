	...
	public class RotationLimitPolygonal : RotationLimit
	{
		...
		private Vector3[] SmoothPoints()
		{
			...
			for (int j = 1; j < array.Length; j += 2)
			{
				...
				array[j] = 0.5f * (array[j - 1] + zero) + scalar * points[j / num].tangentWeight * (zero - b) + scalar * points[j / num].tangentWeight * (array[j - 1] - b2);
			}
		...
		private Quaternion LimitSwing(Quaternion rotation)
		{
			...
			Quaternion lhs = Quaternion.FromToRotation(rotation * axis, vector);
			return lhs * rotation;
		}
