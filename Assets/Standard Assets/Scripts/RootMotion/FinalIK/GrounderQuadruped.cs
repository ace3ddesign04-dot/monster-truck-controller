	...
	public class GrounderQuadruped : Grounder
	{
		...
		private Transform[] InitiateFeet(IK[] ikComponents, ref Foot[] f, int indexOffset)
		{
			...
			for (int i = 0; i < ikComponents.Length; i++)
			{
				...
				solver2.OnPostUpdate = (IKSolver.UpdateDelegate)Delegate.Combine(solver2.OnPostUpdate, new IKSolver.UpdateDelegate(OnPostSolverUpdate));
			}
			return array;
		}
		...
		private void RootRotation()
		{
			if (!(rootRotationWeight <= 0f) && !(rootRotationSpeed <= 0f))
			{
				...
				Vector3 point = forelegSolver.rootHit.point - solver.rootHit.point;
		...
		private void OnSolverUpdate()
		{
			...
			Vector3 a = lastSpineBone.position + forelegSolver.root.up * Mathf.Clamp(forelegSolver.pelvis.heightOffset, float.NegativeInfinity, 0f) - solver.root.up * solver.pelvis.heightOffset;
			Vector3 toDirection = a - pelvis.position;
		...
		private void UpdateForefeetRoot()
		{
			...
			{
				a += forelegSolver.legs[i].transform.position;
			}
			a /= forelegs.Length;
