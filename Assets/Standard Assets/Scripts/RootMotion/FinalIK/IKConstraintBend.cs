...
namespace RootMotion.FinalIK
{
	...
	public class IKConstraintBend
	{
		...
		public void Initiate(IKSolverFullBody solver)
		{
			...
			Vector3 point = Vector3.Cross((bone3.position - bone1.position).normalized, direction);
			defaultChildDirection = Quaternion.Inverse(bone3.rotation) * point;
			initiated = true;
		}
		...
		public void LimitBend(float solverWeight, float positionWeight)
		{
			if (initiated)
			{
				...
				if (changed)
				{
					...
					bone2.rotation = lhs * bone2.rotation;
				}
				if (positionWeight > 0f)
				{
					...
					Quaternion lhs2 = Quaternion.FromToRotation(tangent, vector);
					bone2.rotation = Quaternion.Lerp(bone2.rotation, lhs2 * bone2.rotation, positionWeight * solverWeight);
				}
		...
		public Vector3 GetDir(IKSolverFullBody solver)
		{
			...
			if (bendGoal != null)
			{
				Vector3 lhs = bendGoal.position - solver.GetNode(chainIndex1, nodeIndex1).solverPosition;
				if (lhs != Vector3.zero)
				{
					direction = lhs;
				}
			...
			Vector3 vector = solver.GetNode(chainIndex3, nodeIndex3).solverPosition - solver.GetNode(chainIndex1, nodeIndex1).solverPosition;
			Quaternion rotation = Quaternion.FromToRotation(bone3.position - bone1.position, vector);
			Vector3 vector2 = rotation * (bone2.position - bone1.position);
			if (solver.GetNode(chainIndex3, nodeIndex3).effectorRotationWeight > 0f)
			{
				Vector3 b = -Vector3.Cross(vector, solver.GetNode(chainIndex3, nodeIndex3).solverRotation * defaultChildDirection);
				vector2 = Vector3.Lerp(vector2, b, solver.GetNode(chainIndex3, nodeIndex3).effectorRotationWeight);
			}
			if (rotationOffset != Quaternion.identity)
			{
				Quaternion lhs2 = Quaternion.FromToRotation(rotationOffset * vector, vector);
				vector2 = lhs2 * rotationOffset * vector2;
			}
			if (num <= 0f)
			{
				return vector2;
			}
			return Vector3.Lerp(vector2, direction.normalized, num);
		}
