	...
	public class FBIKChain
	{
		...
		public class ChildConstraint
		{
			...
			public void Solve(IKSolverFullBody solver)
			{
				...
				Vector3 a = solver.chain[chain2Index].nodes[0].solverPosition - solver.chain[chain1Index].nodes[0].solverPosition;
				...
				if (magnitude != nominalDistance && magnitude != 0f)
				{
					...
					Vector3 a2 = a * num;
					solver.chain[chain1Index].nodes[0].solverPosition += a2 * crossFade;
					solver.chain[chain2Index].nodes[0].solverPosition -= a2 * inverseCrossFade;
				}
			}
		}
		...
		public void Reach(IKSolverFullBody solver)
		{
			...
			if (!(vector == Vector3.zero))
			{
				...
				Vector3 a = vector / magnitude * length;
				...
				nodes[0].solverPosition += vector2 * (1f - nodes[0].effectorPositionWeight);
				nodes[2].solverPosition += vector2;
			}
		}
		...
		public Vector3 Push(IKSolverFullBody solver)
		{
			...
			for (int i = 0; i < children.Length; i++)
			{
				vector += solver.chain[children[i]].Push(solver) * solver.chain[children[i]].pushParent;
			}
			nodes[nodes.Length - 1].solverPosition += vector;
			if (nodes.Length < 2)
			{
				return Vector3.zero;
			}
			...
			Vector3 a = nodes[2].solverPosition - nodes[0].solverPosition;
			float magnitude = a.magnitude;
			if (magnitude == 0f)
			{
				return Vector3.zero;
		...
		public void Stage1(IKSolverFullBody solver)
		{
			...
			Vector3 a = nodes[nodes.Length - 1].solverPosition;
			SolveChildConstraints(solver);
			for (int j = 0; j < children.Length; j++)
			{
				Vector3 a2 = solver.chain[children[j]].nodes[0].solverPosition;
				if (solver.chain[children[j]].rootLength > 0f)
				{
					a2 = SolveFABRIKJoint(nodes[nodes.Length - 1].solverPosition, solver.chain[children[j]].nodes[0].solverPosition, solver.chain[children[j]].rootLength);
				}
				if (pullParentSum > 0f)
				{
					a += (a2 - nodes[nodes.Length - 1].solverPosition) * (solver.chain[children[j]].pull / pullParentSum);
				}
			}
			ForwardReach(Vector3.Lerp(a, nodes[nodes.Length - 1].solverPosition, pin));
		}
