	...
	public class FBBIKHeadEffector : MonoBehaviour
	{
		...
		private void Iterate(int iteration)
		{
			if (base.enabled && ik.enabled && base.gameObject.activeInHierarchy && ik.solver.iterations != 0)
			{
				...
				Vector3 b = quaternion * headToBody;
				LerpSolverPosition(ik.solver.bodyEffector, base.transform.position + b, positionWeight * ik.solver.IKPositionWeight, ik.solver.bodyEffector.positionOffset - ik.solver.pullBodyOffset);
				Quaternion rotation = Quaternion.Lerp(Quaternion.identity, quaternion, thighWeight);
				Vector3 b2 = rotation * headToLeftThigh;
				Vector3 b3 = rotation * headToRightThigh;
				LerpSolverPosition(ik.solver.leftThighEffector, base.transform.position + b2, positionWeight * ik.solver.IKPositionWeight, ik.solver.bodyEffector.positionOffset - ik.solver.pullBodyOffset + ik.solver.leftThighEffector.positionOffset);
				LerpSolverPosition(ik.solver.rightThighEffector, base.transform.position + b3, positionWeight * ik.solver.IKPositionWeight, ik.solver.bodyEffector.positionOffset - ik.solver.pullBodyOffset + ik.solver.rightThighEffector.positionOffset);
			}
		}
		...
		private void PostStretching()
		{
			...
			if (num > 0f)
			{
				Vector3 a = Vector3.ClampMagnitude(base.transform.position - ik.references.head.position, maxStretch);
				a *= num;
				stretchDamper = Mathf.Max(stretchDamper, 0f);
				if (stretchDamper > 0f)
				{
					a /= (1f + a.magnitude) * (1f + stretchDamper);
				}
				for (int i = 0; i < stretchBones.Length; i++)
				{
					if (stretchBones[i] != null)
					{
						stretchBones[i].position += a / stretchBones.Length;
					}
				}
			}
