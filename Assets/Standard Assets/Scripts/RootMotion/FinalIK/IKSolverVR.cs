	...
	public class IKSolverVR : IKSolver
	{
		...
		public class Arm : BodyPart
		{
			...
			public void Solve(bool isLeft)
			{
				...
				if (hasShoulder && shoulderRotationWeight > 0f)
				{
					...
					case ShoulderRotationMode.YawPitch:
					{
						...
						Quaternion lhs = Quaternion.AngleAxis(((!isLeft) ? 90f : (-90f)) + num3, chestUp);
						Quaternion rotation2 = lhs * chestRotation;
						Vector3 lhs2 = Quaternion.Inverse(rotation2) * normalized;
						...
						Quaternion lhs3 = Quaternion.AngleAxis((!isLeft) ? 90f : (-90f), chestUp);
						rotation2 = lhs3 * chestRotation;
						...
						lhs2 = Quaternion.Inverse(rotation2) * normalized;
						float num5 = Mathf.Atan2(lhs2.y, lhs2.z) * 57.29578f;
						num5 -= -30f;
						num5 = DamperValue(num5, -15f, 75f);
						Quaternion lhs4 = Quaternion.AngleAxis(0f - num5, rotation2 * Vector3.right);
						Quaternion quaternion = lhs4 * rhs;
					...
					case ShoulderRotationMode.FromTo:
					{
						...
						Vector3 vector = rotation * (solverRotation * shoulder.axis);
						Vector3 vector2 = rotation * (shoulder.solverRotation * shoulder.axis);
				...
				Quaternion lhs5 = Quaternion.FromToRotation(quaternion2 * forearm.axis, hand.solverPosition - forearm.solverPosition);
				RotateTo(forearm, lhs5 * quaternion2, positionWeight);
			...
			private Vector3 GetBendNormal(Vector3 dir)
			{
				...
				if (bendGoalWeight < 1f)
				{
					...
					Quaternion rotation = Quaternion.FromToRotation(down, toDirection);
					Vector3 point = rotation * Vector3.back;
					down = Quaternion.Inverse(chestRotation) * vector;
					toDirection = Quaternion.Inverse(chestRotation) * dir;
					rotation = Quaternion.FromToRotation(down, toDirection);
					point = rotation * point;
					point = chestRotation * point;
					point += vector;
					point -= this.rotation * wristToPalmAxis;
					point -= this.rotation * palmToThumbAxis * 0.5f;
		...
		public class Leg : BodyPart
		{
			...
			protected override void OnRead(Vector3[] positions, Quaternion[] rotations, bool hasChest, bool hasNeck, bool hasShoulders, bool hasToes, int rootIndex, int index)
			{
				...
				if (!initiated)
				{
					...
					if (hasToes)
					{
						...
						bones[1] = new VirtualBone(position2, rotation2);
						bones[2] = new VirtualBone(vector, quaternion);
						bones[3] = new VirtualBone(vector2, quaternion2);
						IKPosition = vector2;
				...
				if (hasToes)
				{
					...
					bones[1].Read(position2, rotation2);
					bones[2].Read(vector, quaternion);
					bones[3].Read(vector2, quaternion2);
				}
				else
				{
					bones[0].Read(position, rotation);
			...
			public override void ApplyOffsets()
			{
				...
				{
					Vector3 point = Vector3.Cross(bendGoal.position - thigh.solverPosition, foot.solverPosition - thigh.solverPosition);
					Quaternion rotation = Quaternion.LookRotation(bendNormal, thigh.solverPosition - foot.solverPosition);
			...
			public void Solve()
			{
				...
				if (hasToes)
				{
					...
					Quaternion lhs = Quaternion.FromToRotation(quaternion * calf.axis, foot.solverPosition - calf.solverPosition);
					RotateTo(calf, lhs * quaternion);
				...
				if (CanStep())
				{
					...
					for (int k = 0; k < footsteps.Length; k++)
					{
						...
						for (int l = 0; l < footsteps.Length; l++)
						{
							if (l != k && !flag)
							{
								if (!(Vector3.Distance(footsteps[k].position, footsteps[l].position) < 0.25f) || !((footsteps[k].position - vector6).sqrMagnitude < (footsteps[l].position - vector6).sqrMagnitude))
								{
									flag2 = GetLineSphereCollision(footsteps[k].position, vector6, footsteps[l].position, 0.25f);
								}
								if (flag2)
								{
									break;
						...
						if (!StepBlocked(footsteps[k].position, vector6, rootBone.solverPosition) && (num11 > num12 || num10 > angleThreshold))
						{
							float num13 = 0f;
							num13 -= num11;
							if (num13 > num7)
		...
		public class Spine : BodyPart
		{
			...
			public override void ApplyOffsets()
			{
				...
				else
				{
					...
					if (num > 0f)
					{
						if (vector3.magnitude < minHeadHeight)
						{
							vector3 = vector3.normalized * minHeadHeight;
						}
					}
					else
					{
			...
			private void SolvePelvis()
			{
				if (pelvisPositionWeight > 0f)
				{
					...
					if (hasChest && hasNeck)
					{
						VirtualBone.SolveTrigonometric(base.bones, pelvisIndex, spineIndex, headIndex, headPosition, bendNormal, pelvisPositionWeight * 0.6f);
						VirtualBone.SolveTrigonometric(base.bones, spineIndex, chestIndex, headIndex, headPosition, bendNormal, pelvisPositionWeight * 0.6f);
						VirtualBone.SolveTrigonometric(base.bones, chestIndex, neckIndex, headIndex, headPosition, bendNormal, pelvisPositionWeight * 1f);
					}
					else if (hasChest && !hasNeck)
					{
						VirtualBone.SolveTrigonometric(base.bones, pelvisIndex, spineIndex, headIndex, headPosition, bendNormal, pelvisPositionWeight * 0.75f);
						VirtualBone.SolveTrigonometric(base.bones, spineIndex, chestIndex, headIndex, headPosition, bendNormal, pelvisPositionWeight * 1f);
					}
					else if (!hasChest && hasNeck)
					{
						VirtualBone.SolveTrigonometric(base.bones, pelvisIndex, spineIndex, headIndex, headPosition, bendNormal, pelvisPositionWeight * 0.75f);
						VirtualBone.SolveTrigonometric(base.bones, spineIndex, neckIndex, headIndex, headPosition, bendNormal, pelvisPositionWeight * 1f);
					}
			...
			private void AdjustChestByHands(ref Quaternion chestTargetRotation, Arm[] arms)
			{
				...
				up.x -= vector2.y;
				up.x *= 0.5f;
				lhs = Quaternion.FromToRotation(Vector3.up, anchorRotation * up);
				chestTargetRotation = lhs * chestTargetRotation;
			}
			...
			private void TranslatePelvis(Leg[] legs, Vector3 deltaPosition, Quaternion deltaRotation)
			{
				...
				a2.y = 0f;
				float d = deltaPosition.y * 0.35f * headHeight;
				deltaPosition += a2 * d;
		...
		private void Solve()
		{
			...
			if (locomotion.weight > 0f)
			{
				...
				float d = leftFootOffset + rightFootOffset;
			...
			else
			{
				...
				{
					spine.InverseTranslateToHead(legs, limited: true, num == 0, bodyOffset, 1f);
					Leg[] array7 = legs;
			...
			for (int num4 = 0; num4 < arms.Length; num4++)
			{
				arms[num4].TranslateRoot(spine.chest.solverPosition, spine.chest.solverRotation);
				arms[num4].Solve(num4 == 0);
			}
			spine.ResetOffsets();
			Leg[] array9 = legs;
			...
			for (int num8 = 0; num8 < legs.Length; num8++)
			{
				float num9 = Vector3.SqrMagnitude(legs[num8].lastBone.solverPosition - legs[num8].bones[0].solverPosition);
				if (num9 < num7)
				{
					supportLegIndex = num8;
					num7 = num9;
				}
			}
		}
