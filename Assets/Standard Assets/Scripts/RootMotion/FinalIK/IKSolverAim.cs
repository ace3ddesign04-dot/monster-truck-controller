	...
	public class IKSolverAim : IKSolverHeuristic
	{
		...
		private void RotateToTarget(Vector3 targetPosition, Bone bone, float weight)
		{
			...
			else
			{
				if (weight >= 0f)
				{
					Quaternion quaternion = Quaternion.FromToRotation(this.transformAxis, targetPosition - transform.position);
					if (weight >= 1f)
					{
				...
				if (poleWeight > 0f)
				{
					Vector3 vector2 = polePosition - transform.position;
					Vector3 tangent = vector2;
					Vector3 normal = this.transformAxis;
