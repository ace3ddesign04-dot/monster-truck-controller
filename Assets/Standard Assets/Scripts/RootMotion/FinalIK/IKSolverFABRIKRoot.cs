	...
	public class IKSolverFABRIKRoot : IKSolver
	{
		...
		private Vector3 GetCentroid()
		{
			...
			for (int j = 0; j < chains.Length; j++)
			{
				if (isRoot[j] && num > 0f)
				{
					vector += (chains[j].ik.solver.bones[0].solverPosition - root.position) * (chains[j].pull / Mathf.Clamp(num, 1f, num));
				}
			}
			return Vector3.Lerp(vector, root.position, rootPin);
		}
	}
}
