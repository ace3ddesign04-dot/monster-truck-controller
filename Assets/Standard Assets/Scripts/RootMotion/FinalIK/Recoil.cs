...
namespace RootMotion.FinalIK
{
	public class Recoil : OffsetModifier
	{
		...
		protected override void OnModifyOffset()
		{
			...
			Quaternion rhs = (!(aimIK != null) || aimIKSolvedLast) ? ik.references.root.rotation : Quaternion.LookRotation(aimIK.solver.IKPosition - aimIK.solver.transform.position, ik.references.root.up);
			rhs = randomRotation * rhs;
			RecoilOffset[] array = offsets;
			foreach (RecoilOffset recoilOffset in array)
			{
				recoilOffset.Apply(ik.solver, rhs, w, length, endTime - Time.time);
			}
			if (!handRotationsSet)
			{
			...
			if (twoHanded)
			{
				Vector3 point = Quaternion.Inverse(primaryHand.rotation) * (secondaryHand.position - primaryHand.position);
				secondaryHandRelativeRotation = Quaternion.Inverse(primaryHand.rotation) * secondaryHand.rotation;
				Vector3 a = primaryHand.position + primaryHandEffector.positionOffset;
				Vector3 a2 = a + handRotation * point;
				secondaryHandEffector.positionOffset += a2 - (secondaryHand.position + secondaryHandEffector.positionOffset);
			}
