...
namespace RootMotion.FinalIK
{
	...
	public class IKSolverFABRIK : IKSolverHeuristic
	{
		...
		protected override void OnUpdate()
		{
			...
			if (XY)
			{
				ref Vector3 iKPosition = ref IKPosition;
				Vector3 position = bones[0].transform.position;
				iKPosition.z = position.z;
			}
		...
		private void BackwardReachLimited(Vector3 position)
		{
			...
			{
				Vector3 a = SolveJoint(bones[i + 1].solverPosition, bones[i].solverPosition, bones[i].length);
				Quaternion lhs = Quaternion.FromToRotation(bones[i].solverRotation * bones[i].axis, a - bones[i].solverPosition);
				Quaternion quaternion = lhs * bones[i].solverRotation;
