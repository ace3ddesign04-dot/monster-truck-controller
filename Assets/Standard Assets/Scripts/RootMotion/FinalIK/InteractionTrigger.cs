	...
	public class InteractionTrigger : MonoBehaviour
	{
		...
		public class CharacterPosition
		{
			...
			public bool IsInRange(Transform character, Transform trigger, out float error)
			{
				...
				Vector3 vector = trigger.position + rotation * offset3D;
				Vector3 b = (!orbit) ? vector : trigger.position;
				Vector3 tangent = character.position - b;
				Vector3.OrthoNormalize(ref normal, ref tangent);
				tangent *= Vector3.Project(character.position - b, tangent).magnitude;
				if (orbit)
				{
				...
				Vector3 tangent2 = rotation * direction3D;
				Vector3.OrthoNormalize(ref normal, ref tangent2);
				if (orbit)
				{
					Vector3 vector2 = vector - trigger.position;
					if (vector2 == Vector3.zero)
					{
						vector2 = Vector3.forward;
					}
					Quaternion rotation2 = Quaternion.LookRotation(vector2, normal);
					tangent = Quaternion.Inverse(rotation2) * tangent;
