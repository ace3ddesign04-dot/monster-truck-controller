...
namespace RootMotion.FinalIK
{
	...
	public class Grounding
	{
		...
		public class Leg
		{
			...
			public void Process()
			{
				...
				case Quality.Simple:
				{
					...
					RaycastHit raycastHit = GetRaycastHit(a + vector);
			...
			private RaycastHit GetCapsuleHit(Vector3 offsetFromHeel)
			{
				...
				Vector3 a = transform.position + vector;
				hitInfo.point = a - up * grounding.maxStep * 2f;
				hitInfo.normal = up;
				Vector3 vector2 = a + grounding.maxStep * up;
				Vector3 point = vector2 + offsetFromHeel;
				if (Physics.CapsuleCast(vector2, point, grounding.footRadius, -up, out hitInfo, grounding.maxStep * 3f, grounding.layers))
				{
			...
			{
				RaycastHit hitInfo = default(RaycastHit);
				Vector3 a = transform.position + offsetFromHeel;
				hitInfo.point = a - up * grounding.maxStep * 2f;
				hitInfo.normal = up;
				...
				Physics.Raycast(a + grounding.maxStep * up, -up, out hitInfo, grounding.maxStep * 3f, grounding.layers);
				return hitInfo;
			}
		...
		public RaycastHit GetRootHit(float maxDistanceMlp = 10f)
		{
			...
			{
				Physics.Raycast(a + up * maxStep, -up, out hitInfo, maxStep * num, layers);
			}
			else
			{
				Physics.SphereCast(a + up * maxStep, rootSphereCastRadius, -this.up, out hitInfo, maxStep * num, layers);
			}
			return hitInfo;
		}
		...
		public Vector3 GetLegsPlaneNormal()
		{
			...
			for (int i = 0; i < legs.Length; i++)
			{
				...
				Quaternion rotation = Quaternion.FromToRotation(tangent, vector2);
		...
		public float GetVerticalOffset(Vector3 p1, Vector3 p2)
		{
			if (useRootRotation)
			{
				Vector3 vector = Quaternion.Inverse(root.rotation) * (p1 - p2);
				return vector.y;
			}
			return p1.y - p2.y;
		}
