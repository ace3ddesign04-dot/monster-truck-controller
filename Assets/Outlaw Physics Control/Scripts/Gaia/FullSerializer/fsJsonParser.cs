...
namespace Gaia.FullSerializer
{
	public class fsJsonParser
	{
		...
		private fsResult TryParseNumber(out fsData data)
		{
			...
			if (!ulong.TryParse(text, NumberStyles.Any, CultureInfo.InvariantCulture, out ulong result2))
			{
				data = null;
				return MakeFailure("Bad UInt64 format with " + text);
			}
		...
		private fsResult TryParseObject(out fsData obj)
		{
			...
			{
				SkipSpace();
				fsResult result = TryParseString(out string str);
				if (result.Failed)
				{
					obj = null;
					return result;
				...
				SkipSpace();
				result = RunParse(out fsData data);
				if (result.Failed)
				{
					obj = null;
					return result;
