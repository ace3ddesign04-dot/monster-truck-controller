...
namespace RootMotion.FinalIK
{
	...
	public class GrounderFBBIK : Grounder
	{
		...
		private void Initiate()
		{
			...
			solver.OnPreUpdate = (IKSolver.UpdateDelegate)Delegate.Combine(solver.OnPreUpdate, new IKSolver.UpdateDelegate(OnSolverUpdate));
			base.solver.Initiate(ik.references.root, feet);
			initiated = true;
		}
		...
		private void OnSolverUpdate()
		{
			...
			if (spineBend != 0f)
			{
				...
				Vector3 a = GetSpineOffsetTarget() * weight;
				spineOffset = Vector3.Lerp(spineOffset, a * spineBend, Time.deltaTime * spineSpeed);
				Vector3 a2 = ik.references.root.up * spineOffset.magnitude;
				for (int i = 0; i < spine.Length; i++)
				{
					ik.solver.GetEffector(spine[i].effectorType).positionOffset += spineOffset * spine[i].horizontalWeight + a2 * spine[i].verticalWeight;
				}
			}
		...
		private void OnDestroy()
		{
			...
			{
				IKSolverFullBodyBiped solver = ik.solver;
				solver.OnPreUpdate = (IKSolver.UpdateDelegate)Delegate.Remove(solver.OnPreUpdate, new IKSolver.UpdateDelegate(OnSolverUpdate));
			}
		}
	}
}
