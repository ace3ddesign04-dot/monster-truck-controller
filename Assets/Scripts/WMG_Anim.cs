...
public static class WMG_Anim
{
	public static void animFill(GameObject obj, float duration, Ease easeType, float animTo)
	{
		...
		DOTween.To(() => comp.fillAmount, delegate(float x)
		{
			comp.fillAmount = x;
		}, animTo, duration).SetEase(easeType).SetUpdate(isIndependentUpdate: false);
	}
	...
	public static void animColor(GameObject obj, float duration, Ease easeType, Color animTo)
	{
		...
		DOTween.To(() => comp.color, delegate(Color x)
		{
			comp.color = x;
		}, animTo, duration).SetEase(easeType).SetUpdate(isIndependentUpdate: false);
	}
	...
	{
		obj.transform.DOLocalRotate(animTo, duration, RotateMode.FastBeyond360).SetEase(easeType).SetUpdate(isIndependentUpdate: false)
	...
	{
		DOTween.To(() => obj.transform.localPosition, delegate(Vector3 x)
		{
	...
	public static void animSize(GameObject obj, float duration, Ease easeType, Vector2 animTo)
	{
		...
		DOTween.To(() => comp.sizeDelta, delegate(Vector2 x)
		{
			comp.sizeDelta = x;
		}, animTo, duration).SetEase(easeType).SetUpdate(isIndependentUpdate: false);
	}
	...
	{
		DOTween.To(() => obj.transform.localScale, delegate(Vector3 x)
		{
	...
	{
		seq.Insert(insTime, DOTween.To(() => obj.transform.localScale, delegate(Vector3 x)
		{
			obj.transform.localScale = x;
		}, animTo, duration).SetEase(easeType).SetUpdate(isIndependentUpdate: false)
			.SetDelay(delay));
	...
	{
		seq.Append(DOTween.To(() => obj.transform.localScale, delegate(Vector3 x)
		{
			obj.transform.localScale = x;
		}, animTo, duration).SetEase(easeType).SetUpdate(isIndependentUpdate: false)
			.SetDelay(delay));
	...
	{
		DOTween.To(getter, setter, animTo, duration).SetUpdate(isIndependentUpdate: false);
	}
	...
	{
		DOTween.To(getter, setter, animTo, duration).SetUpdate(isIndependentUpdate: false).OnUpdate(onUpd);
	}
	...
	{
		DOTween.To(getter, setter, animTo, duration).SetEase(easeType).SetUpdate(isIndependentUpdate: false)
			.OnUpdate(onUpd)
			.OnComplete(onComp);
	}
