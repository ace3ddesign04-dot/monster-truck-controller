...
public class WMG_Ring : WMG_GUI_Functions
{
	...
	public void updateRing(int ringNum)
	{
		...
		WMG_Util.updateBandColors(ref ringColors, graph.outerRadius * 2f, ringRadius - graph.ringWidth, ringRadius, graph.antiAliasing, graph.antiAliasingStrength);
		ringSprite.texture.SetPixels(ringColors);
		ringSprite.texture.Apply();
		if (graph.bandMode)
		{
			SetActive(band, state: true);
			WMG_Util.updateBandColors(ref bandColors, graph.outerRadius * 2f, ringRadius + graph.bandPadding, graph.getRingRadius(ringNum + 1) - graph.ringWidth - graph.bandPadding, graph.antiAliasing, graph.antiAliasingStrength);
			bandSprite.texture.SetPixels(bandColors);
			bandSprite.texture.Apply();
		}
		else
		{
			SetActive(band, state: false);
		}
	}
