...
namespace CustomVP
{
	public class ProgressTracker : MonoBehaviour
	{
		...
		public WaypointRoute.RoutePoint targetPoint
		{
			get;
			private set;
		}

		public WaypointRoute.RoutePoint speedPoint
		{
			get;
			private set;
		}

		public WaypointRoute.RoutePoint progressPoint
		{
			get;
			private set;
		}
		...
		private void Update()
		{
			if (progressStyle == ProgressStyle.SmoothAlongRoute)
			{
				...
				WaypointRoute.RoutePoint routePoint = circuit.GetRoutePoint(progressDistance + lookAheadForTargetOffset + lookAheadForTargetFactor * speed);
				transform.position = routePoint.position;
				Transform transform2 = target;
				WaypointRoute.RoutePoint routePoint2 = circuit.GetRoutePoint(progressDistance + lookAheadForSpeedOffset + lookAheadForSpeedFactor * speed);
				...
				WaypointRoute.RoutePoint progressPoint = this.progressPoint;
				Vector3 vector = progressPoint.position - base.transform.position;
				Vector3 lhs = vector;
				WaypointRoute.RoutePoint progressPoint2 = this.progressPoint;
			...
			else
			{
				...
				target.rotation = circuit.Waypoints[progressNum].rotation;
				this.progressPoint = circuit.GetRoutePoint(progressDistance);
				WaypointRoute.RoutePoint progressPoint3 = this.progressPoint;
				Vector3 vector2 = progressPoint3.position - base.transform.position;
				Vector3 lhs2 = vector2;
				WaypointRoute.RoutePoint progressPoint4 = this.progressPoint;
