...
public class EngineSoundProcessor : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (num2 > 0.01f)
		{
			if (lowOnSound != null)
			{
				lowOnSound.volume = num2 * load;
				lowOnSound.pitch = lowPitchCurve.Evaluate(time);
			}
			lowOffSound.volume = num2 * (1f - load);
		...
		if (num3 > 0.01f)
		{
			if (medOnSound != null)
			{
				medOnSound.volume = num3 * load;
				medOnSound.pitch = medPitchCurve.Evaluate(time);
			}
			medOffSound.volume = num3 * (1f - load);
