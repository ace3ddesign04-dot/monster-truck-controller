			...
			public void AutoLodInternal(LOD[] lods, float lodCulledDistance)
			{
				if (level == maxLevels)
				{
					...
					while (true)
					{
						if (num < lods.Length)
						{
							if (num >= lods.Length - 1 || Mathw.IntersectAABB3Sphere3(box, lods[num].sphere))
							{
								break;
							}
							num++;
					...
					{
						bool enabled = k == num;
						for (int l = 0; l < maxCell.mrList[k].Count; l++)
						{
							maxCell.mrList[k][l].enabled = enabled;
						}
					}
					maxCell.currentLod = num;
					return;
				}
				for (int m = 0; m < 8; m++)
				{
					if (cellsUsed[m])
					{
						cells[m].AutoLodInternal(lods, lodCulledDistance);
					}
				}
			}
