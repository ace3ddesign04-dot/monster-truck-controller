...
namespace PlayFab.Json
{
	...
	public static class PlayFabSimpleJson
	{
		...
		public static string SerializeObject(object json, IJsonSerializerStrategy jsonSerializerStrategy = null)
		{
			...
			return (!SerializeValue(jsonSerializerStrategy, json, _serializeObjectBuilder)) ? null : _serializeObjectBuilder.ToString();
		}
		...
		private static string ParseString(string json, ref int index, ref bool success)
		{
			...
			while (!flag && index != json.Length)
			{
				...
				case '\\':
				{
					...
					if (!(success = uint.TryParse(json.Substring(index, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out uint result)))
					{
						return string.Empty;
					}
					if (55296 <= result && result <= 56319)
					{
						...
						uint result2;
						if (num < 6 || !(json.Substring(index, 2) == "\\u") || !uint.TryParse(json.Substring(index + 2, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out result2) || 56320 > result2 || result2 > 57343)
						{
							success = false;
							return string.Empty;
						}
		...
		private static object ParseNumber(string json, ref int index, ref bool success)
		{
			...
			{
				success = double.TryParse(json.Substring(index, length), NumberStyles.Any, CultureInfo.InvariantCulture, out double result);
				result2 = result;
			}
			else if (text.IndexOf("-", StringComparison.OrdinalIgnoreCase) == -1)
			{
				success = ulong.TryParse(json.Substring(index, length), NumberStyles.Any, CultureInfo.InvariantCulture, out ulong result3);
				result2 = result3;
			}
			else
			{
				success = long.TryParse(json.Substring(index, length), NumberStyles.Any, CultureInfo.InvariantCulture, out long result4);
				result2 = result4;
			}
