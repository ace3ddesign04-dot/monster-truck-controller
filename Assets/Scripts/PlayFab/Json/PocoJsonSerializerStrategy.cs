...
namespace PlayFab.Json
{
	...
	public class PocoJsonSerializerStrategy : IJsonSerializerStrategy
	{
		...
		protected virtual bool TrySerializeUnknownTypes(object input, out object output)
		{
			...
			IDictionary<MemberInfo, ReflectionUtils.GetDelegate> dictionary2 = GetCache[type];
			foreach (KeyValuePair<MemberInfo, ReflectionUtils.GetDelegate> item in dictionary2)
			{
				if (item.Value != null)
				{
					MapClrMemberNameToJsonFieldName(item.Key, out string jsonName, out JsonProperty jsonProp);
