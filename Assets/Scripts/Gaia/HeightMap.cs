...
namespace Gaia
{
	public class HeightMap
	{
		...
		public float GetSlope(float x, float z)
		{
			...
			return Utils.Math_Clamp(0f, 90f, (float)(Math.Sqrt(num * num + num2 * num2) * 10000.0));
		}
		...
		public void Smooth(int iterations)
		{
			for (int i = 0; i < iterations; i++)
			{
				for (int j = 0; j < m_widthX; j++)
				{
					for (int k = 0; k < m_depthZ; k++)
					{
						m_heights[j, k] = Utils.Math_Clamp(0f, 1f, (GetSafeHeight(j - 1, k) + GetSafeHeight(j + 1, k) + GetSafeHeight(j, k - 1) + GetSafeHeight(j, k + 1)) / 4f);
					}
				}
			}
			m_isDirty = true;
