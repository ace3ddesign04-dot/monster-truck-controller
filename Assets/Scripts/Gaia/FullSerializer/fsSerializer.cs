...
namespace Gaia.FullSerializer
{
	public class fsSerializer
	{
		...
		{
			if (_cachedConverters.TryGetValue(type, out fsBaseConverter value))
			{
				return value;
			}
			fsObjectAttribute attribute = fsPortableReflection.GetAttribute<fsObjectAttribute>(type);
			if (attribute != null && attribute.Converter != null)
			{
				...
				fsBaseConverter fsBaseConverter = value;
			...
			if (IsVersioned(data))
			{
				...
				if (versionedType.HasValue)
				{
					...
					if (value.VersionString != asString)
					{
						...
						success = a + InternalDeserialize_3_Inheritance(data, fsVersionedType.ModelType, ref result, out processors);
						if (success.Failed)
						{
							return success;
						}
		...
		private fsResult InternalDeserialize_3_Inheritance(fsData data, Type storageType, ref object result, out List<fsObjectProcessor> processors)
		{
			...
			if (IsTypeSpecified(data))
			{
				...
				else
				{
					...
					{
						success.AddMessage("Ignoring type specifier; a field/property of type " + storageType + " cannot hold an instance of " + type2);
					}
					else
					{
						type = type2;
