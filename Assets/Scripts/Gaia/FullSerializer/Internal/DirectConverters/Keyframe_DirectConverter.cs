...
namespace Gaia.FullSerializer.Internal.DirectConverters
{
	public class Keyframe_DirectConverter : fsDirectConverter<Keyframe>
	{
		...
		protected override fsResult DoDeserialize(Dictionary<string, fsData> data, ref Keyframe model)
		{
			...
			success += DeserializeMember(data, "time", out value);
			model.time = value;
			float value2 = model.value;
			success += DeserializeMember(data, "value", out value2);
			model.value = value2;
			int value3 = model.tangentMode;
			success += DeserializeMember(data, "tangentMode", out value3);
			model.tangentMode = value3;
			float value4 = model.inTangent;
			success += DeserializeMember(data, "inTangent", out value4);
			model.inTangent = value4;
			float value5 = model.outTangent;
			success += DeserializeMember(data, "outTangent", out value5);
